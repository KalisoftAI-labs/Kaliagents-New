...
```javascript
// Line 431-435: Customize order timing
...
// Line 724: Change UPI number
var confirmationMessage = `... Pay UPI Number - *9373332785*`;

// Line 315: Change broadcast time (cron schedule)
cron.schedule('0 9 * * *', async () => {  // 0 9 = 09:00 AM
```
...
- ✅ Change cron timing in `server.js` line 565: `cron.schedule('0 9 * * *',...)`
...
- [ ] Change default PostgreSQL password from `swasth111`
- [ ] Update UPI number in order confirmation message (line 724)
- [ ] Configure proper domain in CORS settings (line 17)
